const Dashboard = {
  ...
  startAutoRefresh() {
    ...
    var self = this;
    this.fastIntervalId = setInterval(function () {
      self.loadFastData();
    }, 5000);
    this.slowIntervalId = setInterval(function () {
      self.loadSlowData();
    }, 30000);
    this.uptimeIntervalId = setInterval(function () {
      self.incrementUptime();
    }, 1000);
  },
  ...
  async loadBattery() {
    try {
      ...
      var chargeInfo = res.charging
        ? I18N.t('dashboard.charging')
        : I18N.t('dashboard.not_charging');
  ...
  async loadIP() {
    try {
      ...
      document.getElementById('stat-local-ip').textContent =
        res.local_ip || '-';
      document.getElementById('stat-public-ip').textContent =
        res.public_ip || '-';
    } catch (err) {
  ...
  async loadNetwork() {
    try {
      ...
      document.getElementById('stat-network-type').textContent =
        res.network_type || '-';
  ...
  async loadClients() {
    ...
    container.innerHTML =
      '<div class="loading">' + I18N.t('dashboard.loading_data') + '</div>';
    ...
    try {
      ...
      if (countEl) {
        countEl.textContent = I18N.t('dashboard.devices_count', {
      ...
      if (clients.length === 0) {
        container.innerHTML =
          '<div class="no-data">' +
          I18N.t('dashboard.no_connected_devices') +
          '</div>';
        return;
      }
      ...
      container.innerHTML =
        '<div class="no-data">' +
        I18N.t('dashboard.failed_load_data') +
        '</div>';
      var countEl = document.getElementById('stat-clients-count');
      if (countEl) {
